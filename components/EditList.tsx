import { View, Text, Switch, TextInput, TouchableOpacity, Pressable, Platform, StyleSheet } from 'react-native'
import { useState } from 'react';
import { DateTimePickerAndroid, DateTimePickerEvent } from '@react-native-community/datetimepicker';
import DateTimePicker from '@react-native-community/datetimepicker';
import { Tlist } from '@/app/(screens)';
...
type modalProps = {
    ...
    currentListItem: Tlist
}

const EditList = ({ isOpen, setIsOpen, editList, currentListItem }: modalProps) => {
    const [name, setName] = useState(currentListItem.name);
    const [isDueDateEnabled, setIsDueDateEnabled] = useState(currentListItem.dueDate!.isEnabled);
    const [dueDate, setDueDate] = useState<Date>(new Date(currentListItem.dueDate!.date));
    const [isCompletionTimeEnabled, setIsCompletionTimeEnabled] = useState(currentListItem.completionTime?.isEnabled);
    const [completionTimeStart, setCompletionTimeStart] = useState<Date | null>(new Date(currentListItem.completionTime!.start));
    const [completionTimeEnd, setCompletionTimeEnd] = useState<Date | null>(new Date(currentListItem.completionTime!.end));
    const [isReminderEnabled, setIsReminderEnabled] = useState(currentListItem.reminder!.isEnabled);
    const [reminder, setReminder] = useState<number>(currentListItem.reminder!.remind);
    ...
    const showMode = (currentMode: 'due_date' | 'start_time' | 'end_time') => {
